...
def query_anthropic(system_prompt: str, user_prompt: str, 
                   model: str = "claude-sonnet-4-20250514",
                   max_tokens: int = 4000) -> str:
    """
    Calls the Anthropic API using the SDK to generate a model response.

    Args:
        system_prompt (str): Instructions for the system role
        user_prompt (str): User input or query
        model (str): Anthropic model name (default: "claude-3-5-sonnet-20241022")
    ...
    """
    try:
        log_step("Anthropic API Call", f"Calling {model}...", "📡")
...
def query_model(system_prompt: str, user_prompt: str, 
    ...
    """
    Wrapper function that uses Anthropic API SDK.
    ...
    Returns:
        str: Generated model response text
    """
    return query_anthropic(system_prompt, user_prompt, model)
